...
interface AppState {
  ...
  setPayoutAnimating: (v: boolean, amount?: number) => void;
  reset: () => void;
}
...
export const useAppStore = create<AppState>((set) => ({
  ...
  setPayoutAnimating: (v, amount) => set({ payoutAnimating: v, lastPayout: amount ?? 0 }),
  reset: () => set({ user: null, policy: null, claims: [], payoutAnimating: false, lastPayout: 0 }),
}));
